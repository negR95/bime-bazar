...
const SelectAddress = (props: Props) => {
  const { $onClose, setAddressToRemove } = props;
    ...
    <section>
      ...
      <div className="flex flex-col max-h-[35dvh] h-full overflow-y-auto">
        {loading ? (
          ...
          addresses
            ?.filter(({ id }) => !removedAdressesIds.has(id))
            .map((address) => (
              <div key={address.id} className="h-14 flex flex-row pr-3 py-2">
                <label className="flex flex-row gap-2 cursor-pointer">
                  <input
                    type="radio"
                    className="self-start appearance-none w-4 h-4 rounded-full border-2 border-[#C2C2C2] bg-white checked:bg-black checked:border-[#C2C2C2] checked:border-[3px] transition"
